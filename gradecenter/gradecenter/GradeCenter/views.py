...
def index(request, andrew = None, course_number = None, year = None):
	...
	try:
		...
		from gradecenter.GradeCenter.viewsP import professor
		return professor(request,course_number,year)
	except Professor.DoesNotExist:
		try:
			andrewi = Student.objects.get(andrewid = request.user)
			from gradecenter.GradeCenter.viewsS import student
...
def logout_view(request):
	logout(request)
	return HttpResponseRedirect(reverse('gradecenter.GradeCenter.views.index',))

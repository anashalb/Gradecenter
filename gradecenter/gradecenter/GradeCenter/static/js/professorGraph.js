...
function params(id, x)
{
	...
	else if (id == "showaverage")
	{
		showaverage = x;
	}
}

function getchart(n,y)
{
		$.ajax({
...
function userGradesChart(ar) {
	...
	var series = {
		...
		{		
			series.name = n;
			for (i=0;i< ar[1][n].length;i++)
			{
				series.data.push(ar[1][n][i]);
			}
...
function autoaverage()
{
	...
	for (i=0; i < grades.length; i++)
	{
		grades[i].value = Math.round(average*maxpoints[i].value);
	}	
	calculate();
}
...
function perform(g)
{
	...
	else
	{
		...
		{
			percentageLeft += parseInt(newWeights[i].value);
		}
		...
		else
		{
			...
			for (i = 0; i < newWeights.length; i++)
			{
				...
				newGrades[i].value = Math.ceil(pointsRequired);
			}
			calculate();			
		}
	}
